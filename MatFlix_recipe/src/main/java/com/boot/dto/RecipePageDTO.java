...
public class RecipePageDTO {
	...
	private RecipeCriteria rc_cri;

	public RecipePageDTO(int rc_total, RecipeCriteria rc_cri) {
		this.rc_total = rc_total;
		this.rc_cri = rc_cri;

//		ex> 3페이지 = 3/10 -> 0.3 -> 1*10 = 10(끝페이지)
//		ex> 11페이지 = 11/10 -> 1.1 -> 2*10 = 20(끝페이지)
		this.rc_endPage = (int) Math.ceil((rc_cri.getRc_pageNum() / 10.0)) * 10;
...
//		ex> total : 70, 현재 페이지 : 3 -> rc_endPage : 10 => 70 * 1.0 / 10 => 7페이지
		int realEnd = (int) Math.ceil((rc_total * 1.0) / rc_cri.getRc_amount());
