...
public class NaviController {
	...
	public String showMap(@RequestParam(required = false) Double startLat,
			@RequestParam(required = false) Double startLng, @RequestParam(required = false) String endAddr,
			Model model) throws Exception {

		if (endAddr == null || endAddr.trim().isEmpty()) {
		...
		String response = kakaoNaviClient.getDirections(startCoord, endAddr);
		...
		log.info("@# showMap endAddr =>" + endAddr);
